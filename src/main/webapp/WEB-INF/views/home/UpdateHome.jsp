...
<script>
	<%String id = (String) session.getAttribute("userid");%>
		$(document).ready(function(){		
			...
			//if amenity의 wifi값이 true이면 checkbox checked
			//for문 돌면서 확인
// 			var temp = 2;
// 			if(temp == 2 ){
// 				$("input[name=amenity_wifi]").attr("checked", true);
// 			}
			$("#logo").click(function(){
...
// 					alert("필수 : " + $('input[name="amenity_essential"]').is(":checked") + ", 와이파이 : " +   $('input[name="amenity_wifi"]').is(":checked"));
					var data = {}
					
					data["accom_id"] = ${avo.accom_id};
			...
			//Amenity List
			var amenList = $.parseJSON(${amenList});
			$.each(${amenList}, function(index, amen){
				$('input[name='+amen.amenity_type+']').prop("checked", JSON.parse(amen.amenity_value));
			});
			
			//select
			$('select[name="room_type"]').val("${avo.room_type}").attr("selected","selected");
			$('select[name="accomodates"]').val("${avo.accomodates}").attr("selected","selected");
			$('select[name="property_type"]').val("${avo.property_type}").attr("selected","selected");
			$('select[name="building_type"]').val("${avo.building_type}").attr("selected","selected");
			$('select[name="bedrooms"]').val("${avo.bedrooms}").attr("selected","selected");
			$('select[name="currency_unit"]').val("${avo.currency_unit}").attr("selected","selected");
			
			$('#maxSleepBedCnt').text("${avo.beds}");
			$('#maxBathCnt').text("${avo.bathrooms}");
			
			//textarea
			$("#roomsDetailInfoDescript").val("${avo.description}");
	...
	<div id="body">
		<div id="contentsWrapper">
			...
			<input type="text" id="address5" name="address5" value="예) 135-919">
 				...
 				<input type="hidden" value="" name="location"> location에 address합쳐서 보내는 form 
					<hr style="border: solid 2px #FF5A5F; width:98%;"> 
 				...
 				<input type="checkbox" name="usableFacility_privatelivingroom"/>개인 거실<br> 
 				...
 				<input type="file" id="roomsImgUpload" name="img"> 
 				<h2>숙소 설명</h2> 
 				<h3>요약</h3> 
 				<textarea rows="10" cols="60" id="roomsDetailInfoDescript" name="description"> 
				</textarea>
 				<h2>숙소의 좋은 점</h2> 
 				<input type="checkbox" name="goodThing_family" value="가족"/>(아이 동반)가족<br> 
 				<input type="checkbox" name="goodThing_group" value="단체"/>단체<br> 
 				<input type="checkbox" name="goodThing_withpet" value="반려동물 동반"/>반려동물 동반<br> 
		
 				<h2>이름 지정</h2> 
 				<input type="text" id="setName" name="name" value="${avo.name}" /> 
 				<h2>기본 요금</h2> 
 				<input type="text" id="payment" name="price" value="${avo.price}" /> 
				...
				<input type="submit" id="updateRoomsBtn" value="숙소 수정">
					<hr style="border: solid 2px #FF5A5F; width:98%;"> 
		</div>
	</div>
	<div id="footer">
		<div id="footerContentsWrapepr">
			<div id="footerContents1">
				...
				<p id="footerP1Right">2조 화이팅!! 모두들 수고많았습니다!! 6달 동안 정말 즐거웠고 남은
					기간도 잘 보내요!!</p>
			</div>
